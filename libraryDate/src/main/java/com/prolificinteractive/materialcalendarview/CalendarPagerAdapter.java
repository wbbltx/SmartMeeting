...
abstract class CalendarPagerAdapter<V extends CalendarPagerView> extends PagerAdapter {
    ...
    //    private int showOtherDates = MaterialCalendarView.SHOW_OUT_OF_RANGE;
//    private int showOtherDates = MaterialCalendarView.SHOW_OTHER_MONTHS;
    ...
    public Object instantiateItem(ViewGroup container, int position) {
        ...
        if (color != null) {
            pagerView.setSelectionColor(color);//设置当天白色字
        }
        if (dateTextAppearance != null) {
            pagerView.setDateTextAppearance(dateTextAppearance);//设置普通字体样式
        }
    ...
    public void setSelectionColor(int color, CalendarDay date) {
        this.color = color;
        for (V pagerView : currentViews) {
            pagerView.setSelectionColor(color, date);
        }
    }
